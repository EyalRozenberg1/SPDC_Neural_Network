...
I = lambda A, n: 2 * n * eps0 * c * torch.abs(A) ** 2  # Intensity
Power2D = lambda A, n, dx, dy: torch.sum(I(A, n)) * dx * dy
...
def PP_crystal_slab(
    ...
    if crystal_profile is None:
        return torch.sign(torch.cos(torch.abs(delta_k) * z))
    else:
        magnitude = torch.abs(crystal_profile)
        phase = torch.angle(crystal_profile)
        if inference:
            max_order_fourier = 20
            poling = 0
            magnitude = magnitude / magnitude.max()
            DutyCycle = torch.arcsin(magnitude) / torch.pi
            for m in range(max_order_fourier):
                if m == 0:
                    poling = poling + 2 * DutyCycle - 1
                else:
                    poling = poling + (2 / (m * torch.pi)) * \
                             torch.sin(m * pi * DutyCycle) * 2 * torch.cos(m * phase + m * torch.abs(delta_k) * z)
            return poling
        else:
            return (2 / torch.pi) * torch.exp(1j * (torch.abs(delta_k) * z)) * magnitude * torch.exp(1j * phase)
...
def HermiteBank(
    ...
    return torch.array(list(Hermite_dict.values())), [*Hermite_dict]
...
def LaguerreBank(
    ...
    get_dict: (True/False) if True, the function will return a dictionary,
              else the dictionary is splitted to basis functions torch.array and list of dictionary keys.
...
def TomographyBank(
    ...
    LG_modes, LG_string = torch.array(list(TOMO_dict.values())), [*TOMO_dict]

    for m in range(len(TOMO_dict) - 1, -1, -1):
        for n in range(m - 1, -1, -1):
            for k in range(len(relative_phase)):
                TOMO_dict[f'{LG_string[m]}+e^j{str(relative_phase[k] / torch.pi)}π{LG_string[n]}'] = \
                    (1 / torch.sqrt(2)) * (LG_modes[m] + torch.exp(1j * relative_phase[k]) * LG_modes[n])

    return torch.array(list(TOMO_dict.values())), [*TOMO_dict]
...
def Hermite_gauss(lam, refractive_index, W0, nx, ny, z, X, Y, coef=None):
    ...
    """
    k = 2 * torch.pi * refractive_index / lam
    z0 = torch.pi * W0 ** 2 * refractive_index / lam  # Rayleigh range
    Wz = W0 * torch.sqrt(1 + (z / z0) ** 2)  # w(z), the variation of the spot size

    invR = z / ((z ** 2) + (z0 ** 2))  # radius of curvature
    gouy = (nx + ny + 1) * torch.arctan(z / z0)
    if coef is None:
        coefx = torch.sqrt(torch.sqrt(2 / pi) / (2 ** nx * math.factorial(nx)))
        coefy = torch.sqrt(torch.sqrt(2 / pi) / (2 ** ny * math.factorial(ny)))
        coef = coefx * coefy
    U = coef * \
        (W0 / Wz) * torch.exp(-(X ** 2 + Y ** 2) / Wz ** 2) * \
        HermiteP(nx, torch.sqrt(2) * X / Wz) * \
        HermiteP(ny, torch.sqrt(2) * Y / Wz) * \
        torch.exp(-1j * (k * (X ** 2 + Y ** 2) / 2) * invR) * \
        torch.exp(1j * gouy)
...
def LaguerreP(p, l, x):
    ...
    if p == 0:
        ...
        return 1 + torch.abs(l) - x
    else:
        return ((2 * p - 1 + torch.abs(l) - x) * LaguerreP(p - 1, l, x) - (p - 1 + torch.abs(l)) * LaguerreP(p - 2, l, x)) / p
...
class Beam(ABC):
    ...
    def __init__(self,
        ...
        self.w = 2 * torch.pi * c / lam  # frequency
        self.k = 2 * torch.pi * ctype(lam * 1e6, T, polarization) / lam  # wave vector
...
class Beam_profile(ABC):
    def __init__(
        ...
        if self.pump_basis.lower() == 'lg':  # Laguerre-Gauss
            self.coef = torch.zeros(len(waist_pump), dtype=torch.float32)
            idx = 0
            for p in range(self.max_mode1):
                for l in range(-self.max_mode2, self.max_mode2 + 1):
                    self.coef[idx] = torch.sqrt(2 * math.factorial(p) / (torch.pi * math.factorial(p + torch.abs(l))))
            ...
            for nx in range(self.max_mode1):
                for ny in range(self.max_mode2):
                    self.coef[idx] = torch.sqrt(torch.sqrt(2 / pi) / (2 ** nx * math.factorial(nx))) * torch.sqrt(torch.sqrt(2 / pi) / (2 ** ny * math.factorial(ny)))
                    idx += 1
    ...
    def _profile_laguerre_gauss(
        ...
        pump_profile = fix_power(pump_profile, self.power, self.refractive_index,
                                 self.crystal_dx, self.crystal_dy)[torch.newaxis, :, :]
        return pump_profile
...
class Crystal_hologram(ABC):
    def __init__(
        ...
        if crystal_basis.lower() == 'ft':  # Fourier-Taylor
            ...
            [X, Y] = torch.meshgrid(self.x, self.y)
            self.coef = torch.zeros(len(r_scale), dtype=torch.float32)
            idx = 0
            for p in range(self.max_mode1):
                for l in range(-self.max_mode2, self.max_mode2 + 1):
                    rad = torch.sqrt(X ** 2 + Y ** 2) / (r_scale[idx] * 1e-5)
                    self.coef[idx] = sp.jv(0, sp.jn_zeros(0, p + 1)[-1] * rad)
                    idx += 1
            ...
            for m in range(self.max_mode1):
                for n in range(self.max_mode2):
                    self.coef = torch.sqrt(torch.sqrt(2 / pi) / (2 ** m * math.factorial(m))) * torch.sqrt(torch.sqrt(2 / pi) / (2 ** n * math.factorial(n)))
    ...
    def _profile_fourier_taylor(
        ...
        [X, Y] = torch.meshgrid(self.x, self.y)
        phi_angle = torch.arctan2(Y, X)
        crystal_profile = 0.
        idx = 0
        for p in range(self.max_mode1):
            for l in range(-self.max_mode2, self.max_mode2 + 1):
                rad = torch.sqrt(X ** 2 + Y ** 2) / (r_scale[idx] * 1e-5)
                crystal_profile += coeffs[idx] * rad ** p * torch.exp(-rad ** 2) * torch.exp(-1j * l * phi_angle)
                idx += 1
    ...
    def _profile_fourier_bessel(
        ...
        for p in range(self.max_mode1):
            for l in range(-self.max_mode2, self.max_mode2 + 1):
                crystal_profile += coeffs[idx] * self.coef[idx] * torch.exp(-1j * l * phi_angle)
                idx += 1
...
def fix_power(
    ...
    """
    output = A * torch.sqrt(power) / torch.sqrt(Power2D(A, n, dx, dy))
    return output
